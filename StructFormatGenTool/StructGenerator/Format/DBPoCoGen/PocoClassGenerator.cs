...
/*
 Original Author : https://github.com/shps951023/PocoClassGenerator
 */

namespace Generator.Format.DBPoCoGen
{
    [Flags]
            ...
            using (var reader = command.ExecuteReader(behavior))
            {
                do
                {
                    ...
                    foreach (DataRow row in schema.Rows)
                    {
                        ...
                        if (generatorBehavior.HasFlag(GeneratorBehavior.DbHander) && !isFromMutiTables)
                        {
                            var isKey = (bool)row["IsKey"];
                            var isAutoIncrement = (bool)row["IsAutoIncrement"];
                            if (isKey && isAutoIncrement)
